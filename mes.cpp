...
int main(int argc, char **argv)
{
    ...
    // Protože počítám že bude větší strom než jen kořen pro jeden prvek to musím odchytit
    if (size == 1)
    {
        ...
        if (file.is_open())
        {
            ...
            std::cout << (int)b << std::endl; // Vstupní řada
            std::cout << (int)b << std::endl; // Seřazená řada
            file.close();
        }
        else
        {
            std::cerr << "Chyba: Nelze otevřít soubor numbers" << std::endl;
